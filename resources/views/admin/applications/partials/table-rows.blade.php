...
<tr class="hover:bg-gray-50 transition-colors">
    <td class="py-4 px-6">
        <input type="checkbox" class="application-checkbox rounded border-gray-300 text-orange-600 focus:ring-orange-500" value="{{ $application->id }}">
    </td>
    <td class="py-4 px-6">
        <div class="flex items-center space-x-3">
            <div class="h-10 w-10 bg-orange-100 rounded-full flex items-center justify-center">
                <span class="text-orange-700 font-semibold text-sm">{{ strtoupper(substr($application->first_name ?? 'U', 0, 1) . substr($application->last_name ?? 'N', 0, 1)) }}</span>
            </div>
            <div>
