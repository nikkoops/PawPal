                ...
                @if(isset($stats['recent_applications']) && $stats['recent_applications']->count() > 0)
                    @foreach($stats['recent_applications'] as $application)
                        <div class="flex items-center space-x-4 p-3 rounded-lg bg-muted/50">
                            ...
                            <div class="flex-1">
                                <p class="text-sm font-medium text-foreground">New application for {{ $application->pet->name ?? 'Pet' }} from {{ $application->user->name ?? 'User' }}</p>
                                <p class="text-xs text-muted-foreground">{{ $application->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    @endforeach
                @else
    ...
    <div class="bg-card/95 backdrop-blur-sm rounded-lg border border-border">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <a href="{{ route('admin.pets.create') }}" class="h-20 flex flex-col items-center justify-center space-y-2 btn-primary">
                    <i data-lucide="heart" class="h-6 w-6"></i>
                    <span>Add New Pet</span>
                </a>
                <a href="{{ route('admin.applications.index') }}" class="h-20 flex flex-col items-center justify-center space-y-2 btn-secondary">
                    <i data-lucide="file-text" class="h-6 w-6"></i>
                    <span>Review Applications</span>
                </a>
                <a href="{{ route('admin.pets.index') }}" class="h-20 flex flex-col items-center justify-center space-y-2 btn-secondary">
                    <i data-lucide="users" class="h-6 w-6"></i>
                    <span>Manage Pets</span>
                </a>
                <a href="{{ route('admin.analytics.index') }}" class="h-20 flex flex-col items-center justify-center space-y-2 btn-secondary">
